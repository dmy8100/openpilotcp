...
static bool volkswagen_mqb_tx_hook(const CANPacket_t *to_send) {
  ...
  const TorqueSteeringLimits VOLKSWAGEN_MQB_STEERING_LIMITS = {
    ...
    .max_rt_delta = 1250,            // 4 max rate up * 50Hz send rate * 250000 RT interval / 1000000 = 50 ; 50 * 1.5 for safety pad = 75
    .max_rt_interval = 250000,     // 250ms between real time checks
    .max_rate_up = 50,              // 2.0 Nm/s RoC limit (EPS rack has own soft-limit of 5.0 Nm/s)
    .max_rate_down = 50,           // 5.0 Nm/s RoC limit (EPS rack has own soft-limit of 5.0 Nm/s)
